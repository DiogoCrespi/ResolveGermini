...
def test_machine_with_cases():
    ...
    machine = TuringMachine('atividade turing/G linguagem_wwr_w_em_ab_estrela.jff')
    
    print("="*70)
    print("TESTANDO MÁQUINA DE TURING PARA LINGUAGEM {wwR | w ∈ {a,b}*}")
    print("="*70)
    ...
    for i, test_case in enumerate(test_data['test_cases']):
        ...
        print(f"\n{'='*70}")
        print(f"TESTE {i+1}/{total_tests}: '{input_string}'")
        print(f"Esperado: {'ACEITA' if expected else 'REJEITA'}")
        print(f"{'='*70}")
        
        accepted, reason = machine.simulate(input_string)
    ...
    print(f"\n{'='*70}")
    print(f"RESULTADO FINAL: {correct_predictions}/{total_tests} testes corretos")
    print(f"Taxa de acerto: {correct_predictions/total_tests*100:.1f}%")
    print(f"{'='*70}")
