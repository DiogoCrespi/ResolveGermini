...
SYSTEM_PROMPT_BASE_FA = (
    ...
    "      \"turing\": {\n"
    "        \"type\": \"turing\",\n"
    "        \"states\": [ { \"id\": 0, \"name\": \"q0\", \"initial\": true, \"final\": false, \"label\": \"Estado inicial\" }, { \"id\": 1, \"name\": \"q1\", \"initial\": false, \"final\": true, \"label\": \"Aceitacao\" } ],\n"
    "        \"transitions\": [ { \"from\": 0, \"to\": 0, \"read\": \"a\", \"write\": \"X\", \"move\": \"R\" }, { \"from\": 0, \"to\": 1, \"read\": \"X\", \"write\": \"X\", \"move\": \"R\" } ]\n"
    "      },\n"
    "      \"cyk_result\": null\n"
    "    }\n"
    "  ]\n"
...
@limits(calls=RATE_LIMIT_PER_MINUTE, period=60)
@retry(wait=wait_exponential(multiplier=1, min=1, max=30), stop=stop_after_attempt(5))
def generate_turing_tests_gpt(questions: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"Erro na geração de testes Turing GPT: {e}")
        return {"questoes": []}
